...
> **Notation:** An open diamond `◇──` on the container side means *has-a (weak)*. The hollow diamond signals the parts can survive without the whole.
...
public class Manager extends Employee {
    ...
    // @Override — replaces Employee's introduce() with a Manager-specific version
    @Override
    public void introduce() {
...
public class Developer extends Employee {
    ...
    // @Override — replaces Employee's introduce() with a Developer-specific version
    @Override
    public void introduce() {
...
public class App {
    ...
    void main() {
        ...
        lars.introduce();          // Manager's version  → "Hi, I'm Lars, Manager of a team of 5 in IT with 12 years of experience."
        sofia.introduce();         // Developer's version → "Hi, I'm Sofia, a Backend Developer (Java) in IT."

        lars.work();               // inherited from Employee → "Lars is working on their tasks."
        sofia.writeCode();         // Developer-only         → "Sofia is writing Java code."

        lars.approveLeave(sofia);  // Manager-only           → "Lars approved leave for Sofia."

        sofia.raiseSalary(5000);   // inherited from Employee → "Sofia's new salary: $80000.0"
        lars.displayInfo();        // inherited from Employee → "Lars | Manager | IT | $90000.0"
    }
...
> You already saw overloading in **Constructors** — `Employee()`, `Employee(name)`, `Employee(name, role, department, salary)` are all overloaded constructors.
> The rule is the same: same name, different parameters, Java picks the right one.
